...
public class EntryTask3 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Solution2.awardTopKHotels(
                "breakfast beach citycentre location metro view staff price",
                "not",
                List.of(1, 2, 1, 1, 2),
                List.of(
    ...
    static class Solution2 {
        public static List<Integer> awardTopKHotels(
            ...
            var formattedPositiveKeywords = positiveKeywords.toLowerCase();
            var formattedNegativeKeywords = negativeKeywords.toLowerCase();
            var positiveKeywordsSet = Set.of(formattedPositiveKeywords.split(" "));
            var negativeKeywordsSet = Set.of(formattedNegativeKeywords.split(" "));


            System.out.println(positiveKeywordsSet );
            System.out.println(negativeKeywordsSet);

            for (var i = 0; i < hotelIds.size(); i++) {
                var review = reviews.get(i);
                var calculatedScore = calculateScore(review, positiveKeywordsSet, negativeKeywordsSet);

                hotelScoreMap.put(
                        hotelIds.get(i),
                        hotelScoreMap.getOrDefault(hotelIds.get(i), 0) + calculatedScore);
            }

            System.out.println(hotelScoreMap);
        ...
        private static int calculateScore(final String review,
                                          final Set<String> positiveKeywordsSet,
                                          final Set<String> negativeKeywordsSet) {
            var score = 0;
            var words = review.toLowerCase().replaceAll("[^a-z0-9 ]", "").split(" ");
